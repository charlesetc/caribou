...
module Make (A : App.S) (D : Display.S) = struct
  ...
  let rec string_of_cursor = function
    | [] -> "[]"
    | x :: rest -> Int.to_string x ^ " :: " ^ string_of_cursor rest
  ...
  let rec increment items =
    ...
    | n :: rest -> (
        ...
        | `Done rest -> `Done (n :: rest)
        | `Bubble_up -> attempt_index items (n + 1) )
  ...
  let rec decrement items =
    ...
    | [ n ] -> (
        ...
        | Some [] -> `Done [ n - 1 ]
        | Some children -> `Done [ n - 1; List.length children - 1 ]
        | None -> `Bubble_up )
    | n :: rest -> (
